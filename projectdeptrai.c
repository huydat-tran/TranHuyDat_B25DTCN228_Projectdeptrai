...
void buyTicket(Ticket *tk, Ticket ticket[], Trip *tp, int tripLength, int *ticketCount){
    ...
    for(i = 0; i < quantity; i++){
        ...
        sprintf(currentTicket->ticketID, "%s%02d", tempTripID, *ticketCount + 1);
	...
	if(index == -1){
		...
		if(tk[index].paymentStatus == -1){
			...
			switch(choice){
				...
				case 2:{
					if(tk[index].paymentStatus == 0){
						tk[index].paymentStatus = -1;
						int tripDx = searchID(tp,tripLength,tk[index].tripID);
						
						if(tripDx != -1){
							tp[tripDx].bookedSeats--;
							printf(">>> Ticket CANCELLED.");
						}
					}else{
						printf("Invalid. Cannot cancel ticket");
					}
